...
ProcessResult Roster::blockArrived(JabberDataBlockRef block, const ResourceContextRef rc) {
    ...
    while (i!=query->getChilds()->end()) {
        ...
        //при push модифицировать ¬—≈ экземпл€ры по bareJid
        Contact::ref contact;
...
// сюда добавить параметр развернута или свернута группа
...
// тут логичней и правильней делать закрыта или открыта группа

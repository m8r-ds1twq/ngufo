...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) 
    {
        ...
        case WM_CREATE:
			...
			SetFocus(rosterWnd->getHWnd()); //чтоб стрелки сразу после запуска работали, без этого непон€тно где фокус

			// не уверен что актуально тут ведь ростер мгновенно не грузитс€ надо бы сделать после Roster Arrived 
			//SendMessage(int.Parse(g), WM_KEYDOWN, VK_A, 0); //дл€ того чтобы не пугать пользователей отсутсвием выделени€ пошлем нажатие кнопки вниз :)

        ...
        while (i!=storage->getChilds()->end()) {
			...
			if (tagName=="conference" & Config::getInstance()->autoMUC) {
				...
				if (b->autoJoin) 
				{
		            ...
		            if (rc->isLoggedIn()) // ну это больше для порядку, хотя может ведь и дисконнект быть неожиданно ? :)
				        rc->jabberStream->sendStanza(joinPresence);	// а вот это как я понял уже входим в конференцию с заданными данными			
					}
            }
        }

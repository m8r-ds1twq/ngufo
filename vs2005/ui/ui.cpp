...
ProcessResult MessageRecv::blockArrived(JabberDataBlockRef block, const ResourceContextRef rc){
    ...
    if (body.length() || subj.length() ) {
		...
		// в эту функцию необходимо добавить параметр таким образом можно сделать разные звуки на разные события :)
		// также необходимо добавить определение на своё сообщение в конференцию, если оно своё то нотифить не надо
		if (c->jid.getResource() != nick) // теперь свои сообщения в конфе не нотяфятся :)
		{
    ...
    bool ascroll=(cv==NULL)? false: cv->autoScroll(); // определяем отрыто ли окно сообщений
